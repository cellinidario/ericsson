...
def main():
    ...
    axes[1].semilogy(ebn0_db - offset, floor(measured), "-o", linewidth=2, label="E2E (DPD + MMSE FFE)")
    axes[1].semilogy(ebn0_db - offset, floor(reference), "-s", linewidth=2, label="ideal PAM-4 (same pipeline)")
    axes[1].semilogy(ebn0_db - offset, floor(theory), "k--", linewidth=1.5, label="theory unipolar PAM-4")
    axes[1].axhline(3.8e-3, color="gray", linestyle="-.", linewidth=1, label="HD-FEC")
